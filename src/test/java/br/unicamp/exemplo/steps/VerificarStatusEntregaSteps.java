...
import br.unicamp.exemplo.VerificadorStatusProduto;
...
public class VerificarStatusEntregaSteps {
	private final String URL_CORREIOS = "http://localhost:8888/correios";
	private final String FILEPATH_STATUS = "statuscorreios.xml";
	private final String FILEPATH_STATUS_NAO_ENCONTRADO = "statuscorreios_naoencontrado.xml";
    ...
    @Before
    public void setUp() {
	    calculadorFretePrazo = new VerificadorStatusProduto(new ClientStatusPedidoCorreios(URL_CORREIOS));
    	throwable = null;
    }
                ...
                post(urlMatching("/correios"))
                        .willReturn(
                                aResponse()
                                        .withStatus(200)
                                        .withHeader("Content-Type", "application/x-www-form-urlencoded")
                                        .withBodyFile(FILEPATH_STATUS_NAO_ENCONTRADO)
                        )
        );
    }
